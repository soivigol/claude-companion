...
function createSftpConfig(store, crypto) {
  ...
  const getChangedFilesForConfig = (projectPath, config) => {
    const all = getChangedFiles(projectPath);
    if (config.context === '.') return all;
  ...
  const clearChangedFilesForConfig = (projectPath, config) => {
    if (config.context === '.') {
      clearChangedFiles(projectPath);
      return;
    }
    const all = store.get(CHANGED_KEY, {});
    const files = all[projectPath] || [];
    const prefix = config.context + '/';
    const remaining = files.filter((f) => !f.startsWith(prefix));
    if (remaining.length) {
      all[projectPath] = remaining;
    } else {
      delete all[projectPath];
    }
    store.set(CHANGED_KEY, all);
  ...
  return {
    ...
    defaultServerConfig,
    getChangedFiles,
    getChangedFilesForConfig,
    addChangedFile,
    clearChangedFiles,
    clearChangedFilesForConfig,
  };
}
