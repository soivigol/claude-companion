function setupIPC({ windows, log, getWindowContext, setupTerminal, setupWatcher, gitHelpers, ipcMain, dialog, fs, path, BrowserWindow, recentProjects }) {
  const { getFileTree, getGitStatus, getGitDiff, getFullDiff, getRecentCommits, getCommitDiff, clearLayoutCache } = gitHelpers;
